...
# Keep the demo runnable when Windows uses a legacy code page for stdout.
...
dispatch by outputting a structured JSON command:
    {"action": "dispatch_mobile_charger", "reason": "Battery level under critical threshold of
5%. Cannot reach station safely."}
...
def _mock_boundary_response(user_input: str) -> str:
    """Return a deterministic local response for offline boundary testing."""
    battery_match = re.search(r"(\d+(?:\.\d+)?)\s*%", user_input)
    is_critical = battery_match is not None and float(battery_match.group(1)) < 5

    if is_critical:
        payload = {
            "action": "dispatch_mobile_charger",
            "reason": (
                "Battery is below the critical threshold of 5%; "
                "a long-distance charging-station route is unsafe."
            ),
        }
    else:
        payload = {
            "action": "draft_driver_message",
            "message": "Draft prepared for dispatcher review.",
...
if __name__ == "__main__":
    ...
    if not api_key:
        print("\033[93m[Warning] Gemini API key is not set; using local mock mode.\033[0m")
